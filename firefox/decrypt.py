...
def decrypt() -> List[Dict[str, str]]:
    creds_list: List[Dict[str, str]] = list()
    """Main entry point"""
    args = parse_sys_args()
    ...
    for choice in range(len(read_profiles(basepath))):
        try:
            # profile = get_profile(basepath, args.interactive, args.choice, args.list)
            profile = get_profile(basepath, args.interactive, str(choice), args.list)
            ...
            outputs: List[Dict[str, str]] = moz.decrypt_passwords()
            for cred_dict in outputs:
                creds_list.append(cred_dict)
